...
for(i = 0; i < numArrays.length; i++){
    ...
    if(i === numArrays.length - 1){
        // if it is then start at index 0 (number 7) and as long as j is less than the last index of the nums array
        //  so in this case is less than 3 continue
        for(j = 0; j < numArrays.length - 1; j++){
            // pushes nums array index i which we know is 3 and grabs the index of j which to start 
            // would be 0 or the number 7 and iterate until the entire array of the last index
            //  of numarrays is complete
